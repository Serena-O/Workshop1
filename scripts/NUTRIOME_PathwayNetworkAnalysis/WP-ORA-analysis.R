...
logfc.cutoff <- 0.58 # corresponds to 50% increase
pvalue.cutoff <- 0.05
...
EnhancedVolcano::EnhancedVolcano(data, title = paste0(nrow(degs), " DEGs"), lab = data$GeneSymbol, 
                                 x = "log2FC", y = "pvalue", pCutoff = pvalue.cutoff, 
                                 FCcutoff = logfc.cutoff, labSize = 3, xlim = c(-3.5,3.5), ylim=c(0,10))
dev.off()
...
res.wp <- clusterProfiler::enrichWP(degs$ENTREZ.ID, organism = "Homo sapiens", pAdjustMethod = "fdr", 
                                    pvalueCutoff = 0.05, minGSSize = 20, maxGSSize = 400)
...
#GeneRatio (A/B): B is the total number of your annotated DEGs found in the pathway database (679/1117 so the expected 65%);
# A is the number of annotated DEGs found in the gene set associated to the pathway of interest
#BgRatio (C/D): C is the number of genes in that pathway/gene set; 
#D is the total number of genes in all the pathways/gene set list in this database (for the species, I think)
...
# ---> the pathway, WP5115 is more up-regulated in Chron's disease patients. Why? 
# Could be due to the ibhibiting protein genes for stopping its expression being downregulated,
# but COULD, cuz gene does not always equal protein making