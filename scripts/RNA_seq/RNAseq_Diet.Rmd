...
```{r}
###the volcano plot already uses the normalized version. this is just a form of extacting the normalized matrix and plotting the PCA of it
# compare normalised counts vs. raw counts 
dds_n <- estimateSizeFactors(dds); 
dds_n <- counts(dds_n, normalized=TRUE)
boxplot(log(dds_n))

# it means that it has been normalized; the variation has been removed
```
