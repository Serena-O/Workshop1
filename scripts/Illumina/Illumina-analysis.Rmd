...
# we are not going to use DESEq because it is not a sequencing data

## overview of the steps
- read in the data
- annotations
- we turn the data into a specific object needed for the the program
- normalize
- filter/annotate again
- make a design matrix
- fit the model based on that design
- then we tell the program which groups we want to compare against each other
- end up with your logfold change, p value and adjusted p value
...
```{r}
project.bgcorrect.norm <- neqc(project, offset = 16) # only for the normalization
```
...
# we are doing these separately because it is limma, but in DESeq, it was done all at once for us
...
design <- model.matrix(~0+Timepoint_diet) # nothing to compare it against so you just add 0; because it is making a linear model
